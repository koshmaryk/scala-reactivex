...
object SelectiveReceive {
  /**
    ...
    * @xparam initialBehavior Behavior to decorate
  ...
  def apply[T: ClassTag](bufferCapacity: Int, initialBehavior: Behavior[T]): Behavior[T] =
    Behaviors.withStash(bufferCapacity) { buffer => intercept(bufferCapacity, buffer, validateAsInitial(initialBehavior)) }
  ...
  private def intercept[T: ClassTag](bufferSize: Int, buffer: StashBuffer[T], started: Behavior[T]): Behavior[T] =
    Behaviors.receive { case (ctx, message) =>
      ...
      if (isUnhandled(next)) {
        ...
        buffer.unstashAll(SelectiveReceive(bufferSize, canonicalize(next, started, ctx)))
      }
    }
